#!/usr/bin/env python
"""
Render the dice shape's BNS output (normals / unblended / one-colour / ...)
using Hakowan.

Reads its input .ply/.obj files from rendering/rendering_results/dice/, the
stable folder that BPS_visualiser.show_bps(shape_name="dice") writes into
(run the notebook's visualisation cell for the dice config first) — no
...
SHAPE_NAME = "dice"
PROXY_FILENAME = "dice500"  # matches surface_config['coarse_patches_id']
...
config = hkw.config()
config.sensor.location = [0, 1.5, 5]

...
hkw.render(vertices + edges + surface, config, filename="rendering_results/powell_sabin.png")

exploded_view = (vertices + edges + surface).transform(
    hkw.transform.Explode("comp_ids", magnitude=0.5)
)
config.sensor.location = [2.5, -2.5, 0]
hkw.render(exploded_view, config, filename="rendering_results/powell_sabin_explode.png")